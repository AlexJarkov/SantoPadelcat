...
export async function initDetail() {
  ...
  if (bc) {
    ...
    if (bcCat) {
      ...
      bcCat.href = `${CONFIG.BASE_URL}/productos/?categoria=${product.category}`;
    }
  }
  ...
  const imgContent = product.images?.[0]
    ? `<img src="${CONFIG.BASE_URL}/${product.images[0]}" alt="${product.name}">`
...
function renderError(container, msg = "Producto no encontrado.") {
  ...
  <div class="detail-error">
    <h2>Oops</h2>
    <p>${msg}</p>
    <a href="${CONFIG.BASE_URL}/productos/" class="btn-primary">Ver catálogo</a>
  </div>`;
}
