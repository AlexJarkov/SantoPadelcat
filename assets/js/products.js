...
export async function fetchProducts() {
  const res = await fetch(`${getPrefix()}data/products.json?v=1.0`);
...
export function renderProductCard(product) {
  ...
  const p = getPrefix();
  const detailURL = `${p}productos/detalle.html?id=${product.id}`;
  const imgContent = product.images?.[0]
    ? `<img src="${p}${product.images[0]}" alt="${product.name}" loading="lazy">`
